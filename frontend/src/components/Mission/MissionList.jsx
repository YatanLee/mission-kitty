...
export default function MissionList({ missions, onToggle, onDelete }) {
  ...
  if (missions.length === 0) {
    return (
      <div className="text-center py-10 text-gray-500">
        ...
        <p className="text-xs mt-1">Add one below or ask Kitty~ 🐾</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {today.length > 0 && (
        <section>
          <h3 className="text-xs text-gray-400 uppercase tracking-widest mb-2">Daily</h3>
      ...
      {weekly.length > 0 && (
        <section>
          <h3 className="text-xs text-gray-400 uppercase tracking-widest mb-2">Weekly</h3>
          <div className="space-y-2">
            <AnimatePresence>
              {weekly.map((m) => (
                <MissionCard key={m.id} mission={m} onToggle={onToggle} onDelete={onDelete} />
